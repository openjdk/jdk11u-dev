...
void JNICALL VMInit(jvmtiEnv *jvmti_env, JNIEnv *env, jthread thr) {
    ...
    for (i = 0; i < RAW_MONITORS_NUMBER; i++) {
        err = jvmti_env->DestroyRawMonitor(monitors[i]);
        if (err != JVMTI_ERROR_NONE) {
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_drrawmon001(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_drrawmon001(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_drrawmon001(JavaVM *jvm, char *options, void *reserved) {
...
JNIEXPORT jint JNICALL Java_nsk_jvmti_DestroyRawMonitor_drrawmon001_check(JNIEnv *env, jclass cls) {
    return result;
}
