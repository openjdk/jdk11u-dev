...
THPSupport::THPSupport() :
    _initialized(false), _mode(THPMode_never), _pagesize(SIZE_MAX) {}
...
void THPSupport::scan_os() {
  ...
  if (f != NULL) {
    ...
    if (::strstr(buf, "[madvise]") != NULL) {
      _mode = THPMode_madvise;
    } else if (::strstr(buf, "[always]") != NULL) {
      _mode = THPMode_always;
    } else {
...
void THPSupport::print_on(outputStream* os) {
  if (_initialized) {
    ...
    os->print_cr("  THP mode: %s",
        (_mode == THPMode_always ? "always" : (_mode == THPMode_never ? "never" : "madvise")));
